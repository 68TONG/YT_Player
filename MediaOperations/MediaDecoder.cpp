...
bool MediaDecoder::setStreams_Type(std::set<AVMediaType> list_type, std::map<int, int> *list_is)
{
    ...
    for(auto it : list_type){
        ...
        if(list_is) list_is->insert({it, index});
...
{
    if(index < 0 || index >= format_ctx->nb_streams) return NULL;
...
void MediaDecoder::decodecRun()
{
    if(is_abort == true) start();
    while(is_abort == false){
        AVPacket *packet = NULL;
        int sic = readPacket(&packet);
        if(sic < 0) return stop();
        if(packet == NULL || sic == false) continue;

        sic = decodecPacket(&packet);
        if(sic < 0) return stop();
    }
    stop();
}

void MediaDecoder::decodecRun_TimeEnd(int64_t end_time)
{
    if(is_abort == true) start();
    while(is_abort == false){
        AVPacket *packet = NULL;
...
void MediaDecoder::decodecRun_Play(int wait_count, int wait_stream_index)
{
    if(is_abort == true) start();
    if(wait_stream_index < 0) wait_stream_index = *codec_ids.begin();
    while(is_abort == false){
...
void MediaDecoder::decodecRun_CountWait(int wait_count, int wait_stream_index)
{
    if(is_abort == true) start();
    if(wait_stream_index < 0) wait_stream_index = *codec_ids.begin();
    while(is_abort == false){
...
int MediaDecoder::decodecPacket(AVPacket **packet_ptr)
{
    ...
    *packet_ptr = NULL;
    ...
    if(sic < 0){
        av_strerror(sic, error, sizeof (error));
        std::cout << format_ctx->url << " MediaDecoder decodecPacket avcodec_send_packet " << error << std::endl;
        av_packet_free(&packet);
        return sic;
    }
    av_packet_free(&packet);
