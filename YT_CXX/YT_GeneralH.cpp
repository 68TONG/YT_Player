...
QString copyFile(const QString &path, const QString &target_path, bool is_cover)
{
    ...
    if(is_info.isFile() == false) return QString();
...
QList<QString> copyFolder(const QString &path, const QString &target_path, bool is_cover, bool is_while)
{
    ...
    if (targetDir.exists() == false) {    //如果目标目录不存在，则进行创建
        if(targetDir.mkdir(targetDir.absolutePath()) == false) return QList<QString>();
    }
    ...
    for (QFileInfo &fileInfo : fileInfoList) {
        if (fileInfo.isDir() && is_while){    // 当为目录时，递归的进行copy
...
QString moveFile(const QString &path, const QString &target_path, bool is_cover)
{
    ...
    if(is_cover && QFile::exists(target_file_path)){
        if(QFile::remove(target_file_path) == false) return QString();
    }

    if(QFile::rename(path, target_file_path) == false) return QString();
    return target_file_path;
}
